...
export const showNotification = async (title, options = {}) => {
  ...
  try {
    // Try to use Service Worker if available (supports actions)
    const registration = await navigator.serviceWorker.getRegistration()
    if (registration && registration.active) {
      ...
      // Fall back to regular Notification API (without actions)
      const notificationOptions = { ...defaultOptions }
      delete notificationOptions.actions // Remove actions for regular Notification API
...
export const showTemplateNotification = async (templateName, customData = {}) => {
  ...
  return await showNotification(template.title, options)
}
