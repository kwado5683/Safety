    ...
    <>
      ...
      <div className="bg-white rounded-lg border border-slate-200 p-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          <div>
            <dt className="text-sm font-medium text-slate-600">Inspection ID</dt>
            <dd className="text-sm text-slate-900 font-mono">INS-{inspection.id}</dd>
          </div>
          <div>
            <dt className="text-sm font-medium text-slate-600">Inspector</dt>
            <dd className="text-sm text-slate-900">{inspection.inspector_id}</dd>
          </div>
          <div>
            <dt className="text-sm font-medium text-slate-600">Started</dt>
            <dd className="text-sm text-slate-900">{formatDate(inspection.started_at)}</dd>
          </div>
          <div>
            <dt className="text-sm font-medium text-slate-600">Completed</dt>
            <dd className="text-sm text-slate-900">{formatDate(inspection.submitted_at)}</dd>
          </div>
        </div>
        ...
        <div className="mt-6 pt-6 border-t border-slate-200">
          <button
            ...
            className="inline-flex items-center px-4 py-2 border border-slate-300 text-slate-700 text-sm font-medium rounded-lg hover:bg-slate-50 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
          >
      ...
      <div className="grid grid-cols-2 lg:grid-cols-5 gap-4">
        ...
        <div className="bg-white rounded-lg border border-slate-200 p-4">
          ...
          <div className="text-sm text-slate-600">Critical Fails</div>
        </div>
      </div>
          ...
          <div className="space-y-4">
            {failedItems.map((item, index) => (
              <div key={index} className="border border-red-200 rounded-lg p-4 bg-red-50">
                ...
                {item.note && (
                  <p className="text-sm text-slate-700 mb-3">{item.note}</p>
                )}
                  ...
                  <div className="flex gap-2 flex-wrap">
                    {item.photos.map((photo, photoIndex) => (
                      <button
                        ...
                        className="w-20 h-20 rounded-lg overflow-hidden border-2 border-slate-200 hover:border-red-400 transition-colors"
                      >
                        <img
                          src={photo}
          ...
          {responses.map((response, index) => (
            <div key={index} className="flex items-center justify-between p-3 border border-slate-200 rounded-lg">
              <div className="flex-1">
                ...
                {response.note && (
                  <p className="text-sm text-slate-600">{response.note}</p>
                )}
              </div>
