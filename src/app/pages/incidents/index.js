...
- Fetches from /pages/api/incidents/list with query params.
...
export default function IncidentsIndexPage() {
  ...
  async function load() {
    ...
    const res = await fetch(`/pages/api/incidents/list?${query.toString()}`)
    if (res.ok) {
  ...
  return (
    <Layout>
      <div className="flex items-center justify-between">
        ...
        <Link href="/pages/incidents/new" className="rounded-full bg-indigo-600 text-white text-sm px-4 py-2">New</Link>
      </div>
              ...
              items.map((it) => (
                <tr key={it.id} className="border-t border-neutral-200">
                  <td className="px-4 py-3">
                    <Link href={`/pages/incidents/${it.id}`} className="text-indigo-600 underline">{it.title}</Link>
                  </td>
