...
- Loads incident via /pages/api/incidents/get?id=...
- Lists actions via /pages/api/incidents/actions?incidentId=...
...
export default function IncidentDetailsPage() {
  ...
  async function load() {
    ...
    const [incRes, actRes] = await Promise.all([
      fetch(`/pages/api/incidents/get?id=${id}`),
      fetch(`/pages/api/incidents/actions?incidentId=${id}`),
    ])
    if (incRes.ok) {
  ...
  async function createAction() {
    const res = await fetch('/pages/api/incidents/actions', {
      method: 'POST',
  ...
  async function updateAction(idToUpdate, payload) {
    const res = await fetch(`/pages/api/incidents/actions?id=${idToUpdate}`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
  ...
  async function deleteAction(idToDelete) {
    const res = await fetch(`/pages/api/incidents/actions?id=${idToDelete}`, {
      method: 'DELETE',
    })
    if (res.ok) load()
  }
