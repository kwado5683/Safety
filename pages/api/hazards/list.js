/*
Description: Pages API route to list hazards with pagination and filters.
- Requires authentication via Clerk.
- Supports status, category, and severity filters.

Pseudocode:
- If method !== GET → 405
- Check auth; if no user → 401
- Parse page, pageSize, status, category, severity from req.query
- Query hazards with optional filters
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const { page = '1', pageSize = '10', status, category, severity } = req.query || {}
    ...
    if (countError) {
      return res.status(500).json({ error: 'Failed to count hazards' })
    }
    ...
    if (severity) rowsQuery = rowsQuery.eq('severity', severity)
