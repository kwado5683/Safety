/*
Description: Pages API route to list trainings with pagination and filters.
- Requires authentication via Clerk.
- Supports category and status filters.

Pseudocode:
- If method !== GET → 405
- Check auth; if no user → 401
- Parse page, pageSize, category from req.query
- Query trainings with optional filters
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (countError) {
      return res.status(500).json({ error: 'Failed to count trainings' })
    }
